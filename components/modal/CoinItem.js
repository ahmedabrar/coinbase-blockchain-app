...
const CoinItem = ({
  ...
  useEffect(() => {
    const getBalance = async () => {
      ...
      thirdWebTokens.map((thirdWebToken) => {
        if (thirdWebToken.address === token.contactAddress) {
  ...
  return (
    <Wrapper
      style={{ backgroundColor: selectedToken === token.name && "#141519" }}
      onClick={() => {
    ...
    >
      ...
      <IsSelected>
        {selectedToken.contactAddress === token.contactAddress && <FaCheck />}
      </IsSelected>
    </Wrapper>
  );
};
